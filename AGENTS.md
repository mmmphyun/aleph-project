...
### 4.1 작업 착수 5단계 표준 파이프라인
모든 에이전트와 작업자는 새 작업 착수 시 다음 5단계를 엄격히 준수한다:
1. **1단계 (main 최신화)**: `git checkout main && git pull origin main`으로 최신 커밋 동기화.
2. **2단계 (GitHub Issue 선발행)**:
   - `node scripts/get_my_tasks.js`로 노션 티켓 링크 확인 (부재 시 웹에서 카드 링크 복사).
   - `gh issue create --title "<타입>(<직무>): <제목>" --body "- Notion Task: <노션URL>\n- 브랜치: feat/<직무>-<기능명>"` 실행하여 이슈 번호 확보.
3. **3단계 (작업 브랜치 분기)**: `git checkout -b feat/<직무>-<기능명>` 생성 후 개발 착수.
4. **4단계 (로컬 통합 검증)**: PR 생성 전 `powershell .\scripts\check.ps1` 단일 게이트 100% 통과 (우회 플래그 사용 절대 금지).
5. **5단계 (PR 발행 및 1:1 짝꿍 리뷰어 지정)**: `gh pr create --reviewer "<짝꿍ID>"`로 PR 생성 및 상호 리뷰 요청.

### 4.2 직무별 1:1 상호 짝꿍 리뷰어 매핑
...
- 작업 완료 및 PR 제출 전 다음 단일 검증 스크립트를 반드시 100% 통과해야 한다:
  ...
  ```
  (내부적으로 `ruff check`, `ruff format --check`, `pytest`를 일괄 수행하며, `--no-respect-gitignore` 등 우회 플래그 사용은 엄격히 금지된다.)
