...
* **핵심 원칙 (도메인 소유권 준수)**:
  * 각 도메인 담당자가 작성한 모듈(네트워크의 SG 정책, 클라우드 B의 Slack 알림, 보안의 룰/LLM)의 로직을 침범하지 않고, 각 모듈이 단 한 줄의 코드 수정 없이 결합될 수 있도록 **런타임 오케스트레이터, IaC 모듈, CI/CD 배관, 로컬 Mock 하네스**의 아키텍처 완성도에 집중합니다.
