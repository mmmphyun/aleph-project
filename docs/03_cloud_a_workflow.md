...
* **핵심 원칙 (직무 지분 비침범)**:
  * 각 도메인 담당자가 작성한 모듈(네트워크의 SG 정책, 클라우드 B의 슬랙 알림, 보안의 룰/LLM)의 로직을 침범하지 않고, 담당자들의 코드가 플러그인처럼 꽂혀서 돌아가는 **"런타임 오케스트레이터, IaC, CI/CD 배관, 로컬 Mock 하네스"**에 본인의 엔지니어링 깊이를 집중함.
