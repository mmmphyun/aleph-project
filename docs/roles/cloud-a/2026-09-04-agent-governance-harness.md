...
- **문제 인식**: 전공자 1인과 비전공자 3인으로 구성된 팀에서 AI 코딩 에이전트(Cursor, Windsurf, Claude Code 등)를 도입할 경우, 에이전트가 타 팀원의 코드를 무단 수정하거나 사전 합의되지 않은 데이터 구조를 작성하여 런타임 통합 실패를 유발할 위험이 큼.
- **목적**:
