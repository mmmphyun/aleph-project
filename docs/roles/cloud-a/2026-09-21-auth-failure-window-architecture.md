...
* **지연 로그 지원 범위와 의도된 시나리오적 타협 (Deliberate Architectural Trade-off)**:
  * **지원 보장**: 기준 시각 대비 인접 버킷($curr \pm 1$, $\pm 300$초) 내의 역순/지연 배치는 100% 완벽 탐지합니다.
  * **시나리오적 타협**: 단일 배치 내 타임스탬프 스팬이 600초를 초과하는 장기 지연 혼합 배치(예: $t=299$ 4건과 $t=901$ 1건 공존)는 마지막 시각 901을 기준으로 스캔하므로 299초 이벤트가 제외됩니다.
  * **타협 채택 근거**:
    1. 본 시스템의 핵심 목표는 **'10초 관통 실시간 자동 대응'**이며, 정상 스트리밍 환경에서 CloudWatch Agent의 전송 지연은 최대 수 초 단위입니다. 600초 이상 네트워크 단절 후 일괄 플러시되는 시나리오는 실시간 침해사고 파이프라인의 전제 범위를 벗어납니다.
    2. 이를 지원하기 위해 단일 배치 내 모든 고유 시간대별로 DynamoDB 3-버킷 스캔 루프를 순회할 경우, 단일 Lambda 호출에서 Read RTT가 2~5배 선형 증가하여 파이프라인 전체의 10초 SLA를 위협합니다.
    3. 따라서 오버엔지니어링을 배제하고 실시간 대응 SLA와 DB RTT를 엄격히 사수하기 위해, **'인접 버킷($\pm 300$초) 내 실시간 역순 수렴'**으로 지원 경계를 닫는 실용적 아키텍처 타협을 채택했습니다.
