...
외부 LLM API를 동기 호출하지 않는 결정론적 Fallback 구조를 적용했다. 이 선택은 10초 실시간 대응 SLA를 보장하고, 외부 API 지연·장애가 차단 파이프라인 전체를 지연시키는 위험을 제거하기 위한 아키텍처 결정이다. 그 결과 데모 환경에서 재현성과 단위 테스트 가능성을 확보했다.
...
- 실행 전제: 프로젝트 가상환경 활성화(`.\.venv\Scripts\Activate.ps1`) 후 검증 스크립트 실행
- 실행 명령: `powershell -ExecutionPolicy Bypass -File .\scripts\check.ps1`
- 결과: 전체 로컬 게이트 통과
- 상세 결과: Ruff Lint 통과, Ruff Format 검증 통과, Pytest `187 passed, 2 skipped, 1 warning`
- 판정: 보안 문서 변경 이후에도 공통 계약 및 직무별 단위 테스트를 포함한 전체 검증 게이트를 통과함
...
검증은 가상환경 활성화 후 `scripts/check.ps1` 단일 게이트로 완료했다. 이번 PR은 보고서 전용 변경이므로 런타임 코드, 공통 계약, 클라우드 A 차단 오케스트레이터, 클라우드 B 알림 모듈에는 영향을 주지 않는다.
