# CloudShield: 팀원 온보딩 퀵스타트 가이드 (Onboarding Guide)

본 문서는 CloudShield 프로젝트에 참여하는 각 도메인 담당자가 로컬 개발 환경을 설정하고, 코딩 에이전트를 활용하여 안전하게 기여(Contribution)할 수 있도록 안내하는 표준 가이드입니다.
...
## 5. Pull Request(PR) 제출 전 필수 로컬 검증
...
## 6. PR 작성 및 Notion 자동 연동

1. GitHub 저장소에서 `main` 브랜치를 대상으로 Pull Request를 생성합니다.
2. PR 템플릿에 맞춰 아래 항목을 작성합니다:
   * 관련 노션 태스크 링크 및 이슈 번호.
   * **기술 원리 3줄 요약 (필수)**: 에이전트가 작성한 코드의 핵심 로직과 동작 원리를 담당자가 직접 기술합니다.
3. PR이 머지(Merge)되면 GitHub Actions가 Notion 칸반 보드의 카드 상태를 자동으로 `Done`으로 변경하고, 작성한 3줄 요약을 성과 기록으로 자동 동기화합니다.
