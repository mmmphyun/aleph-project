...
- **내용**:
  - 타깃 인스턴스 또는 공격 경로 상에서 `tcpdump`를 실행하여 트래픽 캡처.
    ...
    ```
  - Wireshark로 `.pcap` 파일을 열어 정상 접속과 무차별 대입 공격 시 발생하는 TCP 플래그(SYN, RST) 비율, 접속 주기, 세션 지속 시간 비교 분석.
- **산출물**:
...
### 5단계: 침해 인스턴스 격리 정책 명세
- **내용**:
  - 공격에 노출되거나 침해된 EC2가 다른 내부 시스템을 공격하지 못하도록 차단하는 보안 그룹 설계.
  - **격리 보안 그룹 규칙**: 인바운드 전체 차단(분석용 보안팀 사내 IP만 허용), 아웃바운드 전체 차단.
- **산출물**:
