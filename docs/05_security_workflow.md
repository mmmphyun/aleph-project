...
## 1. 개요 및 역할 정의
보안 담당은 인입된 로그 데이터에서 1차 이상징후를 판별하는 결정론적 룰(정규식/임계치)을 정의하고, 이를 글로벌 보안 프레임워크(MITRE ATT&CK)와 매핑하며, LLM Structured Output을 활용해 표준화된 JSON 침해사고 분석 결과를 도출한다.
...
flowchart TD
    ...
    S2 --> S3[3단계: LLM Structured Output 스키마 정의]
...
- **내용**:
  ...
  - **룰 1: 단일 계정 Brute Force** (동일 IP에서 동일 계정 대상 실패 5회 이상).
  - **룰 2: 다중 계정 Password Spraying** (동일 IP에서 2개 이상의 서로 다른 계정 실패).
...
### 3단계: LLM Structured Output 스키마 정의 (Pydantic)
- **내용**:
  - 부트캠프에서 학습한 구조화 출력 기법을 적용하여, LLM 응답이 정해진 규격의 JSON 객체로 반환되도록 Pydantic 모델 선언.
    ```python
    ...
    ```
- **산출물**:
  - 데이터 모델 정의 코드(`models.py`).

### 4단계: 프롬프트 엔지니어링 및 분석기 개발
- **내용**:
  - 시스템 프롬프트(System Prompt)에 보안 관제 전문가 역할을 부여하고 Few-shot 예시를 추가.
  - OpenAI/Gemini SDK의 Structured Output(또는 Tool Calling) 기능을 호출하여 로그 텍스트를 `IncidentReport` 객체로 변환하는 단일 함수 구현.
    ```python
...
- **내용**:
  ...
  - 정상 접속 로그가 들어왔을 때 오탐(False Positive)으로 차단되지 않고 `risk_level: "LOW"`로 평가되는지 임계치 튜닝.
- **산출물**:
