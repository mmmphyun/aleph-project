...
## 1. 개요 및 역할 정의
보안 담당은 인입된 로그 데이터에서 1차 이상징후를 판별하는 결정론적 룰(정규식/임계치)을 정의하고, 이를 MITRE ATT&CK 프레임워크와 매핑하여 10초 관통 대응을 위한 표준 `IncidentReport` 계약 객체로 승격한다. 또한 실시간 차단 완료 후 사후 심층 분석을 위해 비동기 LLM 구조화 출력 파이프라인을 연계한다.
...
- **내용**:
  - 각 탐지 룰을 MITRE ATT&CK 공격 기법과 1:1로 매핑하여 공격 식별자 및 대응 체계 정형화.
